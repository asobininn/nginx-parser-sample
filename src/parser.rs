...
use winnow::{
    ...
    combinator::{alt, cut_err, eof, preceded, repeat, seq, terminated},
    token::take_while,
};

use crate::{ast::*, error::SyntaxError};
...
pub fn parse(source: &str) -> ModalResult<ConfigAst<'_>> {
    let mut input = Stateful {
        input: RawInput::new(source),
        state: ParseState::default(),
    };
    config(&mut input)?;
    Ok(input.state.finish())
}

fn config<'s>(input: &mut Input<'s>) -> ModalResult<()> {
    preceded(ws0, terminated(directives, eof)).parse_next(input)
}

fn directive<'s>(input: &mut Input<'s>) -> ModalResult<DirectiveId<'s>> {
    alt((block_directive, simple_directive)).parse_next(input)
}

fn simple_directive<'s>(input: &mut Input<'s>) -> ModalResult<DirectiveId<'s>> {
    let ((header, semicolon_span), span) = seq!(directive_header, _: ws0, ';'.span())
        .with_span()
        .parse_next(input)?;
...
fn block_directive<'s>(input: &mut Input<'s>) -> ModalResult<DirectiveId<'s>> {
    let ((header, open_brace_span), span) = seq!(directive_header, _: ws0, '{'.span())
        .with_span()
        .parse_next(input)?;
    let id = input.state.begin_block(header, open_brace_span, span);

    input.state.parents.push(id);
    let close_res = block_content(input);
...
fn block_content<'s>(input: &mut Input<'s>) -> ModalResult<Span> {
...
fn directives<'s>(input: &mut Input<'s>) -> ModalResult<()> {
    repeat(0.., terminated(directive, ws0)).parse_next(input)
}

fn directive_header<'s>(input: &mut Input<'s>) -> ModalResult<DirectiveHeader<'s>> {
...
fn directive_name<'s>(input: &mut Input<'s>) -> ModalResult<(&'s str, Span)> {
    take_while(1.., |c: char| c.is_ascii_alphanumeric() || c == '_')
        .with_span()
        .parse_next(input)
}

fn arg<'s>(input: &mut Input<'s>) -> ModalResult<Arg<'s>> {
    alt((bare_arg, quoted_arg)).parse_next(input)
}

fn bare_arg<'s>(input: &mut Input<'s>) -> ModalResult<Arg<'s>> {
    take_while(1.., |c: char| {
...
fn quoted_arg<'s>(input: &mut Input<'s>) -> ModalResult<Arg<'s>> {
    alt((
...
fn quoted_with<'s>(quote: char, input: &mut Input<'s>) -> ModalResult<Arg<'s>> {
    preceded(
        quote,
        cut_err(terminated(
            escaped(
...
fn ws0(input: &mut Input<'_>) -> ModalResult<()> {
    multispace0.void().parse_next(input)
}

fn ws1(input: &mut Input<'_>) -> ModalResult<()> {
